...
>Recomendamos que o projeto configurado para receber solicitações por email não tenha mais de um tópico da fila. Se as solicitações enviadas se destinam a recursos ou projetos diferentes, você deve direcioná-las ou movê-las manualmente após o envio.
...
* Se houver documentos anexados ao email, eles serão anexados ao problema no Workfront.
  ...
  >
  >* Se seu ambiente tiver acesso ao armazenamento em nuvem herdado do Workfront e do Adobe para documentos, as solicitações herdarão seu tipo de armazenamento de documento do projeto, independentemente do que o administrador do Workfront selecionou para sua preferência de armazenamento padrão na área Configuração. Para obter informações, consulte [Visão geral do gerenciamento de documentos para projetos e objetos relacionados](/help/quicksilver/manage-work/projects/manage-projects/manage-documents-on-projects.md).
