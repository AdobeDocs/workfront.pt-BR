...
<table style="table-layout:auto"> 
 ...
 <tbody> 
  <tr> 
   ...
   <td> <p>Taxa de burndown ideal com base em quando a iteração começou.</p> <p>Esta linha não será exibida se o escopo da iteração nunca for alterado (horas ou pontos nunca são adicionados ou removidos).</p> <p>Esta linha é exibida como sem graça quando o trabalho é feito em um dia de folga. Para obter mais informações, consulte <a title="Usando o Gráfico Burndown Ágil" href="#how-days-off-affect-the-burndown-chart" class="MCXref xref">Como os dias de folga afetam o gráfico de burndown</a>.</p> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td> <p>Alteração no intervalo de datas (a duração da iteração aumenta ou diminui).</p> <p>Um ponto azul é exibido no meio de qualquer dia em que a duração da iteração foi alterada.</p> </td> 
  </tr> 
  <tr> 
   <td role="rowheader"> <img src="assets/agile-iteration-burndown-scope.png" alt="Ponto verde para trabalhar queimado"> </td> 
   <td> <p>Um ponto verde ou vermelho é exibido na taxa de burndown real sempre que o trabalho é gravado. (Quando a taxa de burndown real naquele dia é vermelha, o ponto é vermelho; quando a taxa de burndown real naquele dia é verde, o ponto é verde.)</p> <p>O trabalho é gravado quando ocorrer qualquer uma das seguintes situações:</p> 
    <ul> 
     <li> A [!UICONTROL Percentual de Conclusão] é aumentada na matéria.<br>[!UICONTROL Percentual Concluído] é aumentado quando: 
      <ul> 
       <li> <p>Alterado manualmente</p> </li> 
       <li> <p>O número de pontos ou horas é atualizado na matéria</p> </li> 
      </ul></li>  
...
Equipes ágeis podem incorporar dias de folga específicos da equipe definindo um agendamento alternativo (conforme descrito no artigo [Usar um agendamento de equipe alternativo para gráficos de burndown](../../../agile/use-scrum-in-an-agile-team/burndown/use-alt-team-schedule-burndown-charts.md)). Esse cronograma alternativo é refletido no gráfico de burndown de qualquer iteração atribuída à equipe. A programação alternativa afeta somente o gráfico de burndown.

Os dias de folga serão refletidos no gráfico de burndown somente se:

* O trabalho foi registrado anteriormente em um dia de folga. (O dia em que o trabalho foi registrado é exibido.)

  Quando o trabalho é registrado em um dia de folga:

   * Nenhum trabalho registrado é incluído no cálculo do burndown ideal, pois a equipe não está programada para realizar nenhum trabalho.
   * As linhas de detalhamento ideais (a linha azul sólida e a linha azul tracejada) são exibidas como planas no gráfico de detalhamento para qualquer dia em que o trabalho tenha sido concluído ou no dia em que o gráfico de detalhamento estiver sendo exibido (se você estiver visualizando em um dia de folga).
   * O trabalho registrado é incluído no cálculo de outras estatísticas de burndown, como a conclusão estimada e a média de pontos ou horas por dia.

* Você está exibindo o gráfico de burndown em um dia de folga. (O dia que você está exibindo é mostrado no gráfico de burndown.)
* Você conclui o trabalho restante total da iteração em um dia de folga.

  Quando um usuário conclui o trabalho total restante da iteração em um dia de folga, o campo [!UICONTROL Conclusão Estimada] exibe a data em que a iteração foi concluída.

  Ao planejar a iteração, se você definir a data de término da iteração para um dia não útil e a iteração estiver rastreando para terminar no prazo, a data de [!UICONTROL Conclusão Estimada] será definida para o último dia útil anterior à data de término da iteração definida (porque o trabalho não está programado para ser gravado em dias não úteis).
