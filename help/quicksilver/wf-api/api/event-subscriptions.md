---
title: Assinaturas de eventos
content-type: api
navigation-topic: Event Subscriptions resources
description: Recursos de assinaturas de eventos
author: Becky
feature: Workfront API
role: Developer
exl-id: b604d8fe-3a5b-4217-9a08-df269826f112
TQID: https://experienceleague.adobe.com/DWHydmTwK7aj7-HlM6mIZOea1fpG9LoM1Ry-rvTD7yQ
product_v2: id: c4a86a5d-6562-4fc6-aa00-bfa25833aed9
role_v2: id: ff6a42d2-313e-452e-93a6-792e4fad9ff8
source-git-commit: 55a9d9feae8cc1128e3427a8874414ba734dd467
workflow-type: tm+mt
source-wordcount: 51
ht-degree: 100%

---

# Assinaturas de eventos

A seguir, há vários recursos sobre Assinaturas de evento no Adobe Workfront:

* [API de assinatura de evento](../../wf-api/general/event-subs-api.md)
* [Práticas recomendadas de assinatura de evento](../../wf-api/general/event-sub-best-practice.md)
* [Requisitos de entrega de assinatura de evento](../../wf-api/general/setup-event-sub-endpoint.md)
* [Campos de recursos de assinatura de evento](../../wf-api/api/event-sub-resource-fields.md)
* [Tentativas de assinatura de evento](../../wf-api/api/event-sub-retries.md)
* [Controle de versão da assinatura de evento](/help/quicksilver/wf-api/general/event-subs-versioning.md)
* [Filtrar mensagens de assinatura de evento](../../wf-api/api/filter-event-sub-messages.md)
* [Formato de mensagem de saída para assinaturas de evento](../../wf-api/api/message-format-event-subs.md)
* [Perguntas frequentes: assinaturas de eventos](../../wf-api/general/event-subs-faq.md)
