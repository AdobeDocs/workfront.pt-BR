...
* O Adobe Workfront Planning registra com objetos de outros aplicativos.
   ...
   * Adobe Experience Manager
      ...
      * Ativos
        ...
        Você pode conectar os seguintes tipos de ativos do Workfront Planning:
...
<td> 
   ...
   <td> 
   <p> Além do Adobe Workfront, você deve ter o seguinte, se quiser conectar registros com objetos dos seguintes aplicativos:</p>
...
<tr> 
  ...
  <tr> 
   ...
   <p><span class="preview">Gerenciar permissões para o registro do qual você está se conectando</span></p>
   <p><span class="preview">Exibir ou ter permissões mais altas para o registro ao qual você está se conectando</span></p>
...
Depois de criar uma conexão entre um tipo de registro e objetos do Adobe Experience Manager, você pode conectar registros individuais a objetos do Experience Manager. Os campos de ativos que você conectou pelo Experience Manager ao criar a conexão são preenchidos automaticamente no tipo de registro que você vinculou.
...
Para conectar registros a objetos do Adobe Experience Manager:
...
1. Clique no cartão de um tipo de registro para abrir a página do tipo de registro.
1. Selecione uma exibição de **Tabela** no menu suspenso **Exibir**, no canto superior esquerdo da página de tipo de registro.
...
1. (Condicional) Depois de conectar o tipo de registro selecionado aos objetos do AEM, vá para o campo conectado em um registro e clique no campo ou clique em **Conectar** para adicionar objetos do Experience Manager ao registro e, em seguida, clique no ícone **+**.

   >[!TIP]
   >
   >  Você pode clicar no ícone **+** no campo de objeto vinculado na página de detalhes do registro para conectar ativos ao registro.
...
1. Na guia **Assets**, clique para selecionar alguns dos seguintes tipos de ativos:
...
1. Quando você terminar de escolher os ativos ou fragmentos de conteúdo, clique em **Selecionar**. <!-- we might change this to Connect-->
   ...
   * Os ativos ou fragmentos de conteúdo selecionados do Experience Manager são adicionados ao campo de registro vinculado.
   * Os campos vinculados (ou campos de pesquisa) são preenchidos com informações dos objetos conectados do Experience Manager.
     ...
     Todas as informações existentes nos campos dos ativos ou fragmentos de conteúdo do Experience Manager são exibidas nos campos vinculados ou de pesquisa adicionados automaticamente ao Planning.

     Se você conectar tipos de registro do GenStudio for Performance Marketing do espaço de trabalho do GenStudio no Planning com objetos do AEM, poderá exibir informações do GenStudio para campanhas, regiões, perfis, produtos e canais no AEM.

     Você deve ter uma licença do Adobe GenStudio for Performance Marketing além de uma licença do AEM e do Workfront Planning para exibir o espaço de trabalho do GenStudio no Workfront Planning.
...
1. (Opcional e condicional) Vá para o tipo de registro vinculado ao Experience Manager a partir de e clique no nome de um ativo no campo de registro vinculado. Os detalhes do Experience Manager do ativo são exibidos em uma janela pop-up. <!--not sure if this is also possible for fragments??-->
   ...
   * Data de modificação
     ...
     >[!TIP]
     >
     >A janela de detalhes do objeto Experience Manager é exibida em qualquer lugar no Planning onde o objeto conectado é exibido. Por exemplo, é possível abrir as janelas de detalhes para objetos AEM na janela de tabela ou na área Detalhes de um registro.

1. (Opcional e condicional) Para abrir a página de registro do ativo ou fragmento do Experience Manager no Experience Manager, vá para a página de tipo do registro do qual você está vinculando, clique no nome de um ativo no campo de registro vinculado para abrir a janela pop-up e clique no ícone ![Abrir ativo no ícone do AEM](assets/open-asset-icon.png) do **Abrir no AEM** para abrir o ativo. <!--not sure if this is also possible for fragments??-->

   Isso abre o ativo Experience Manager no Adobe Experience Manager Assets.

1. (Opcional e condicional) Na exibição de tabela do tipo de registro, passe o mouse sobre o cabeçalho da coluna do ativo ou fragmento vinculado do Experience Manager, clique no menu suspenso e clique em **Editar campos de pesquisa**. <!--not sure if this is also possible for fragments??-->

1. Adicionar campos de objeto do Experience Manager Assets da área <!--not sure if this is also possible for fragments??--> de **campos não selecionados**
...
1. (Opcional) Clique no ícone **Abrir em nova guia** ![Abrir detalhes em um novo ícone de guia](assets/open-details-in-a-new-tab-icon.png) para abrir a página do registro em uma nova guia do navegador.
1. (Opcional e condicional) Clique no nome de um tipo de registro na navegação estrutural do cabeçalho da página de registro para acessar outro tipo de registro na mesma hierarquia. As hierarquias devem existir para o tipo de registro do registro que você está conectando antes que você possa exibi-las na navegação estrutural. Para obter informações, consulte [Criar hierarquias do espaço de trabalho](/help/quicksilver/planning/architecture/create-workspace-hierarchies.md).
