...
A fim de tornar [!DNL Adobe Workfront Fusion] Solicitação HTTP(S) para servidores que exigem uma autorização OAuth 2.0, primeiro é necessário criar uma conexão OAuth. [!DNL Adobe Workfront Fusion] A garante que todas as chamadas feitas com essa conexão tenham os cabeçalhos de autorização apropriados e atualizem automaticamente os tokens associados quando necessário.

[!DNL Workfront Fusion] O é compatível com os seguintes fluxos de autenticação do OAuth 2.0:
...
* [Instruções para criar uma conexão com o Google no HTTP >[!UICONTROL Marca] um módulo de solicitação OAuth 2.0](#instructions-for-creating-a-connection-to-google-in-the-http-make-an-oauth-20-request-module)
...
### Instruções gerais para a criação de uma conexão no [!UICONTROL HTTP] > [!UICONTROL Fazer uma solicitação do OAuth 2.0] módulo

1. Crie um cliente OAuth no [!DNL target] serviço com o qual você deseja [!DNL Adobe Workfront Fusion] para se comunicar. Essa opção provavelmente foi encontrada no [!UICONTROL Desenvolvedor] seção do serviço fornecido.

   1. Ao criar um cliente do, insira o URL apropriado no `[!UICONTROL Redirect URL]` ou `[!UICONTROL Callback URL]` campo:

      | Américas / APAC | https://app.workfrontfusion.com/oauth/cb/oauth2 |
      |---|---|
      | EMEA | https://app-eu.workfrontfusion.com/oauth/cb/oauth2 |

   1. Após criar o cliente, o serviço fornecido exibe duas chaves: `[!UICONTROL Client ID]` e `[!UICONTROL Client Secret]`. Alguns serviços chamam isso de `[!UICONTROL App Key]` e `[!UICONTROL App Secret]` . Salve a chave e o segredo em um local seguro, para que você possa fornecê-los ao criar a conexão no Workfront Fusion.

1. Localize o `[!UICONTROL Authorize URI]` e `[!UICONTROL Token URI]` na documentação da API do serviço fornecido. Esses são endereços de URL através dos quais [!DNL Workfront Fusion] se comunica com a [!DNL target] serviço. Os endereços são usados para autorização OAuth.

   >[!NOTE]
   >
   >Se o serviço usar Fluxo implícito, você precisará somente do `[!UICONTROL Authorize URI]`.

   >[!INFO]
   >
   >**Exemplo:** Endereços Yahoo:
   >
   >* Autorizar URI:
   >
    ...
    <tbody> 
     ...
     <tr> 
      ...
      <td> <p>Selecione o fluxo para obter tokens.</p> 
       <ul> 
        ...
        <li><strong>[!UICONTROL Implícito]</strong>: Insira o <code>[!UICONTROL Authorize URI]</code> na documentação da API do serviço.</li> 
       </ul> </td> 
     </tr> 
     <tr> 
     ...
     <tr> 
      ...
      <td> <p>Adicione os parâmetros que deseja incluir na chamada de autorização. Os seguintes parâmetros padrão são sempre incluídos automaticamente e não precisam ser adicionados.</p> <p>Parâmetros padrão:</p> 
       <ul> 
        <li> <p><strong>[!UICONTROL tipo_de_resposta]</strong> </p> <p> <code>code </code>para o [!UICONTROL Fluxo de código de autorização] e <code>token </code>para [!UICONTROL Fluxo implícito]</p> </li> 
     ...
     <tr> 
      ...
      <td> <p>Adicione os parâmetros que deseja incluir na chamada de token. Os seguintes parâmetros padrão são sempre incluídos automaticamente e não precisam ser adicionados.</p> <p>Parâmetros padrão:</p> 
       ...
       </ul> <p>Nota:  <p>O padrão OAuth 2.0 é compatível com pelo menos dois métodos de autenticação de cliente durante essa etapa (<code>[!UICONTROL client_secret_basic]</code> e <code>[!UICONTROL client_secret_post]</code>). [!DNL Workfront Fusion] envia automaticamente a ID do cliente e a senha especificadas por meio da <code>[!UICONTROL client_secret_post]</code> método. Portanto, esses parâmetros são incluídos automaticamente como parte do corpo da solicitação de token. </p> <p>Para obter mais informações sobre a autenticação OAuth 2.0, consulte <a href="https://tools.ietf.org/html/rfc6749">A estrutura de autorização OAuth 2.0</a>.</p> </p> </td> 
     </tr> 
     <tr> 
      ...
      <td> <p>Adicione os parâmetros que deseja incluir na chamada de token. Os seguintes parâmetros padrão são sempre incluídos automaticamente e não precisam ser adicionados.</p> <p>Parâmetros padrão:</p> 
       <ul> 
        ...
        <li> <p><strong>[!UICONTROL_refresh_token]</strong>: o token de atualização mais recente obtido pelo serviço ao qual você está se conectando</p> </li> 
...
### Instruções para criar uma conexão com o [!DNL Google] no [!UICONTROL HTTP] >[!UICONTROL Criar um módulo de solicitação OAuth 2.0]

O exemplo a seguir mostra como usar a variável [!UICONTROL HTTP] > [!UICONTROL Criar um OAuth 2.0] módulo de solicitação ao qual se conectar [!DNL Google].

1. Verifique se você criou um projeto, definiu as configurações do OAuth e gerou suas credenciais conforme descrito em [Conectar [!DNL Adobe Workfront Fusion] para [!DNL Google Services] uso de um cliente OAuth personalizado](../../../workfront-fusion/connections/connect-fusion-to-google-using-oauth.md).
...
1. Insira os seguintes valores:
   ...
   <table style="table-layout:auto">  
    ...
    <tbody> 
     ...
     <tr> 
      <td role="rowheader">[!UICONTROL Escopo] </td> 
      <td> <p>Adicionar escopos individuais. Para obter mais informações sobre escopos, consulte <a href="https://developers.google.com/identity/protocols/oauth2/scopes">Escopos do OAuth 2.O para [!DNL Google] APIs</a> no [!DNL Google] documentação.</p> </td> 
     </tr> 
     <tr> 
      <td role="rowheader">[!UICONTROL Separador de escopo] </td> 
     ...
     <tr> 
      <td role="rowheader">[!UICONTROL ID do Cliente] </td> 
      <td> <p>Insira seu [!DNL Google] ID do cliente. </p> <p>Para criar uma ID do cliente, consulte <a href="../../../workfront-fusion/connections/connect-fusion-to-google-using-oauth.md#create2" class="MCXref xref">Criar credenciais do OAuth</a> in <a href="../../../workfront-fusion/connections/connect-fusion-to-google-using-oauth.md" class="MCXref xref">[!DNL Connect Adobe Workfront Fusion] para [!DNL Google Services] uso de um cliente OAuth personalizado</a>.</p> </td> 
     </tr> 
     <tr> 
      <td role="rowheader">[!UICONTROL Segredo do Cliente]</td> 
      <td> <p>Insira seu [!DNL Google] Segredo do cliente. </p> <p>Para criar um segredo do cliente, consulte <a href="../../../workfront-fusion/connections/connect-fusion-to-google-using-oauth.md#create2" class="MCXref xref">Criar credenciais do OAuth</a> in <a href="../../../workfront-fusion/connections/connect-fusion-to-google-using-oauth.md" class="MCXref xref">[!DNL Connect Adobe Workfront Fusion] para [!DNL Google] Serviços que usam um cliente OAuth personalizado</a>.</p> </td> 
     </tr> 
     <tr> 
      <td role="rowheader"> <p>[!UICONTROL Autorizar parâmetros]</p> </td> 
      <td> <p>Adicionar <code>[!UICONTROL access_type]</code> - <code>[!UICONTROL offline] </code>par de valor-chave.</p> <p> <img src="assets/google-authentication-http.png"> </p> <p>Observação: se você tiver problemas de autenticação, por exemplo, com a atualização de token, tente adicionar o <code>[!UICONTROL prompt] </code>- <code>[!UICONTROL consent] </code>par de valor-chave.</p> </td> 
     </tr> 
    </tbody> 
   </table>
...
### Instruções para conexão com o [!DNL Microsoft Graph API] por meio da [!UICONTROL HTTP] > [!UICONTROL Fazer uma solicitação do OAuth 2.0] módulo

Para obter instruções sobre [!DNL Microsoft Graph API], consulte [Chame o [!DNL MS Graph REST API] por meio da [!DNL Adobe Workfront Fusion] [!UICONTROL HTTP] > [!UICONTROL Fazer uma solicitação do OAuth 2.0] módulo](../../../workfront-fusion/connections/call-the-ms-graph-rest-api.md).

## Configuração do módulo de solicitação OAuth 2.0

Quando tiver estabelecido uma [!DNL Oauth 2].0 conforme descrito em [Criação de uma conexão para um [!DNL OAuth] solicitação](#creating-a-connection-for-an-oauth-request), continue configurando o módulo conforme desejado. Todos os tokens de autorização são incluídos automaticamente nessa solicitação e em qualquer outra solicitação que use a mesma conexão.
...
<table style="table-layout:auto">  
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td> <p> Habilite esta opção para solicitar uma versão compactada do site.</p> <p>Isso adiciona um <code>[!UICONTROL Accept-Encoding]</code> cabeçalho para solicitar conteúdo compactado.</p> </td> 
  </tr> 
  <tr> 
