...
1. (Opcional) Para editar informações limitadas sobre um projeto, clique em **Detalhes do projeto** no painel esquerdo. Se quiser editar todas as informações sobre o projeto, prossiga para a Etapa 5. <!--accurate?!-->
   ...
   1. (Opcional e condicional) Quando uma área for recolhida, clique na **seta apontando para a direita** ![seta apontando para a direita](assets/right-pointing-arrow.png) ao lado de cada área para expandir a área que você deseja editar. Ou clique na **seta apontando para a direita** no canto superior direito para expandir todas as áreas.

      Para obter mais informações sobre como editar informações na guia Detalhes do projeto, consulte os seguintes artigos:
...
Você pode definir quais permissões os usuários recebem para o projeto, bem como as tarefas e problemas no projeto quando eles interagem com qualquer um desses tipos de objeto.

>[!IMPORTANT]
>
>Modificar as permissões para o projeto, tarefas e problemas ao editar o projeto estabelece como as permissões são concedidas quando os usuários interagem com esses tipos de objeto após a alteração do acesso.
>
>A alteração dessa configuração não modifica retroativamente as permissões dos usuários para esses tipos de objetos, para itens aos quais os usuários já foram atribuídos ou incluídos.
