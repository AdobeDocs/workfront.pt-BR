...
>[!NOTE]
>
>Você pode ter até 30 campos de conexão para um tipo de registro.
...
1. Procure um tipo de registro ou selecione um dos seguintes:
   ...
   * Um tipo de registro de outro espaço de trabalho que foi configurado para se conectar a outros espaços de trabalho.
     ...
     >[!TIP]
     >
     >Você pode usar a seguinte combinação de teclado para abrir a caixa de pesquisa global em qualquer página do Workfront Planning e procurar tipos de registro:
     >
     >* CTRL+K para Windows
     >* ⌘+K para Mac
     >
   ...
   * Um tipo de objeto de **Solicitação original** da seção **Tipos de Objeto do Workfront**.

     O campo de conexão da solicitação original criado exibirá o nome da solicitação do Planning submetida que criou o objeto. O nome da solicitação também é visível no campo **Assunto** da solicitação.

     >[!NOTE]
     >
     >Você deve criar formulários de solicitação e publicá-los para enviar solicitações do Planning para criar registros no Workfront Planning.
     >
     ...
     ![Seleção de conexão da solicitação original](assets/original-request-connection-selection.png)


   * **Experience Manager** da seção **Aplicativos Adobe**, para se conectar ao AEM Assets ou a Fragmentos de Conteúdo.

     ![Seleção de conexão do AEM Assets](assets/aem-assets-connection-selection-041326.png)
...
1. <span class="preview">Selecione **Tornar esta conexão dependente** para adicionar dependências entre este campo de conexão e o campo criado no tipo de registro conectado. </span>

   <span class="preview">Para obter informações, consulte [Gerenciar conexões dependentes](/help/quicksilver/planning/architecture/manage-dependent-connections.md).</span>

1. Selecione o **Criar campo correspondente no tipo de registro vinculado** na seção **Tipo de conexão**.

   >[!TIP]
   >
   ><span class="preview">A seção **Tipo de conexão** foi substituída pela seção **Configurações de conexão** ao conectar tipos de registro do Planning. </span>
   ...
   >[!TIP]
   >
   >* Além do limite de 30 campos de conexão para um tipo de registro, há um limite de 500 campos para um tipo de registro. Recomendamos manter essa configuração desativada, especialmente para tipos de registros taxonômicos, para evitar atingir esse limite.
   >
...
1. (Condicional) Ao se conectar ao Experience Manager, escolha um repositório no menu suspenso **Repositório do Experience Manager** na seção **Vincular ativos do repositório a seguir**. Este campo é obrigatório. Somente repositórios aos quais você tem acesso no Experience Manager são exibidos nesse campo.
...
1. <span class="preview">(Opcional e condicional) Se você selecionou **Tornar esta conexão dependente**, clique em **Regras de filtragem de registro** para criar um filtro para limitar os valores dos valores do campo conectado quando ambos os campos de registro conectados forem exibidos no mesmo tipo de registro. </span>

   <span class="preview">Para obter mais informações, consulte [Gerenciar conexões dependentes](/help/quicksilver/planning/architecture/manage-dependent-connections.md). </span>
...
1. Selecione os **Selecionar campos de pesquisa** ou <span class="preview">clique no menu **Campos de pesquisa**</span> para escolher e adicionar campos do tipo de registro ao qual você está se conectando.

   Os campos de pesquisa são campos associados ao tipo de registro ou objeto ao qual você está se conectando. Vinculá-los exibe informações do registro ou objeto ao qual você está vinculando no registro a partir do qual você está vinculando. <!--No more: This is selected by default.-->
   ...
   >* Somente os seguintes campos são suportados para Fragmentos de conteúdo do AEM: <!--this information is also in "Connect record types overview"-->
...
1. Clique em **Adicionar campos** para salvar as alterações.
   ...
   * Ao vincular tipos de registro uns aos outros, um campo de registro vinculado também é adicionado ao tipo de registro ao qual você está vinculando, somente quando você habilita a configuração **Criar campo correspondente no tipo de registro vinculado**. O nome do campo de registro vinculado no tipo de registro vinculado é o nome do tipo de registro que você vincula.
     ...
     > Recomendamos não criar links para tipos de registros taxonômicos, pois há um limite de 500 campos, além do limite de 30 campos conectados para cada tipo de registro.
