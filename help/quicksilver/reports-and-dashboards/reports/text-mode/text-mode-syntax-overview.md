...
visão geral de sintaxe para exibições e agrupamentos {#valuefield-syntax-overview-for-views-and-groupings}

```
Valuefield=
```

é uma linha principal de código em exibições e agrupamentos que identifica o objeto que você está referenciando diretamente.

A sintaxe para campos de referência direta é idêntica para agrupamentos e visualizações.

As seguintes regras se aplicam ao referenciar objetos do Workfront usando um
...
visão geral de sintaxe para exibições e agrupamentos {#valueexpression-syntax-overview-for-views-and-groupings}
...
ao criar exibições e agrupamentos no modo texto quando quiser fazer referência a um cálculo entre 2 ou mais campos.
...
* É possível usar um curinga em uma
...
A segunda linha de código mais importante em uma visualização ou agrupamento é o

```
valueformat=
```

linha. Isso informa ao Workfront em que formato retornar o valor especificado na variável

```
valuefield
```

ou linhas de expressão de valor. Embora você possa usar vários formatos para a variável
