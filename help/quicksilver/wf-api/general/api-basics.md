...
1. Altere o URL para /attask/api/v9.0/project/search.\
...
1. Altere o URL de volta para **/attask/api/v9.0/project/search**.
...
Observe que a solicitação /attask/api/v9.0/project?id=.. é igual à solicitação `/attask/api/v9.0/project/...` solicitação.
...
<pre>GET /attask/api/v9.0/task/search?name=Planning<br>&amp;name_Mod=contains<br>&amp;OU:1:portfólio:name=FixedAssets<br>&amp;OU:1:portfólio:name_Mod=eq<br>&amp;OU:1:assignedTo:name=Steve<br>&amp;OU:1:assignedTo:name_Mod=cicontains<br>&amp;OU:2:parent:name=Tarefa Final<br>&amp;OU:2:parent:name_Mod=eq
</pre>
...
<pre>/attask/api/v9.0/user/search?username=testuser@workfront.com</pre>passe o parâmetro do URL em um filtro, como mostrado no exemplo a seguir:
<pre>/attask/api/v9.0/user/search?filters={"nome do usuário":"testuser@workfront.com"}</pre>
...
<pre>/attask/api/v9.0/task/search?map=true</pre>retorna uma resposta indexada pela ID semelhante ao seguinte:
...
<pre>/attask/api/v9.0/task/search?fields=planStartDate,priority</pre>retorna uma resposta semelhante à seguinte:
...
<pre>/attask/api/v9.0/issue/search?fields=owner</pre>Se mais informações forem necessárias, é possível solicitar um campo aninhado usando a sintaxe de dois pontos. Por exemplo, a solicitação a seguir pesquisa todos os problemas juntamente com o nome, ID, título e número de telefone do proprietário
<pre>/attask/api/v9.0/issue/search?fields=owner:title,owner:phoneNumber</pre>e retorna o seguinte: 
...
<pre>/attask/api/v9.0/project/search?fields=tasks</pre>A solicitação a seguir obtém atribuições de tarefa:
<pre>/attask/api/v9.0/task/search?fields=atribuições</pre>
...
<pre>/attask/api/v9.0/project/search?fields=DE:CustomText</pre>que retornaria
...
<pre>/attask/api/v9.0/project/search?fields=parameterValues</pre>retorna dados semelhantes aos seguintes:
...
<pre>GET /attask/api/v9.0/project/count?status=CUR</pre>retorna o número de resultados no seguinte formato:
...
<pre>GET /attask/api/v9.0/hour/report?project:name_1_GroupBy=true&amp;hours_AggFunc=sum</pre>que retorna o seguinte resultado
...
<pre>GET /attask/api/v9.0/project/search?$$LIMIT=200</pre>Para garantir confiabilidade e desempenho para outros locatários no sistema, o limite máximo de resultados permitidos por query é de 2000 objetos. Tentar especificar um limite maior resultará em uma mensagem de erro IllegalArgumentException . 
...
<pre>GET /attask/api/v9.0/project/123abcxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx?method=put &amp;updates={ accessRules: [ {accessID: 'abc123', accessObjCode: 'USUÁRIO', coreAction: 'VIEW'} ] }</pre>Como alternativa, para compartilhar somente com uma nova pessoa e manter as permissões existentes intactas:
<pre>GET /attask/api/v9.0/project/123abcxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx/share?method=put&amp;accessID=abc123&amp;accessObjCode=USER&amp;coreAction=VIEW</pre>Para recuperar as regras de acesso existentes:
<pre>GET /attask/api/v9.0/project/123abcxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx?fields=accessRules:*</pre>
...
<pre>POST /attask/api/v9.0/project?name=New Project</pre>A resposta inclui o projeto recém-criado, juntamente com sua nova ID e quaisquer outros campos especificados.
...
<pre>POST /attask/api/v9.0/upload</pre>A API espera que o tipo de conteúdo seja dados de várias partes/formulários. O nome do parâmetro para o arquivo deve ser uploadFile. O servidor retorna os seguintes dados JSON:
...
<pre>POST /attask/api/v9.0/document?updates={<br>    name: aFileName,<br>    identificador: abc...123, (identificador do upload do arquivo)<br>    docObjCode: PROJ, (ou TAREFA, OPTASK etc.)<br>    objID: abc...123,<br>    currentVersion:{version:v1.0,fileName:aFileName}<br>}</pre>
...
<pre>PUT /attask/api/v9.0/project/4c7..?name=Novo nome do projeto <br>PUT /attask/api/v9.0/project?id=4c7..&amp;name=Novo nome do projeto</pre>Como a atualização requer uma ID, essa operação falhará (sem inserção) se o objeto não existir no servidor.
...
<pre>PUT /attask/api/v9.0/project/4c7..?update= <br>{<br>     name: "Nome do novo projeto", <br>     status: "CUR", <br>     ... <br>}</pre>
...
<pre>PUT /attask/api/v9.0/task/4c7...?update= <br>{<br>    atribuições: [ <br>        { <br>            assignedToID: "2222...54d0, <br>            assignPercent: 50,0 <br>        },{ <br>            roleID: "1111...54d0"<br>        } <br>    ] <br>}</pre>
...
<pre>PUT /attask/api/v9.0/project/4c7..?update= <br>{ <br>    queueDef: { <br>        isPublic: 1 <br>    } <br>}</pre>
...
<pre>PUT /attask/api/v9.0/project/4c7..?action=calculateTimeline<br><br>ou<br><br>PUT /attask/api/v9.0/project/4c7../calculateTimeline </pre>

### Mover objetos

A seguir, é exibida a sintaxe para mover uma tarefa de um projeto para outro:
<pre>PUT /attask/api/v9.0/task/4c7../move?projectID=5d8..</pre>Um exemplo para cada tipo de ação é fornecido aqui: (??)
<pre>PUT /attask/api/v9.0/project/1234/approveApproval<br><br>PUT /attask/api/v9.0/project/1234/calculateFinance<br><br>PUT /attask/api/v9.0/project/1234/calculateLinha do tempo<br><br>PUT /attask/api/v9.0/project/1234/calculateDataExtension<br><br>PUT /attask/api/v9.0/project/1234/callApproval<br><br>PUT /attask/api/v9.0/project/1234/rejectApproval<br><br>PUT /attask/api/v9.0/task/1234/move<br><br>PUT /attask/api/v9.0/workitem/1234/markViewed</pre>Somente a ação de mover requer a identificação de atributos adicionais para especificar o projeto onde o item de trabalho será movido.

Este é um exemplo de cada tipo de ação: 
<pre>PUT /attask/api/v9.0/project/1234?method=put&amp;updates={accessRules:[{accessID: 'abc123', accessObjCode: 'USUÁRIO', coreAction: 'VIEW'}]}</pre>

### Compartilhamento de objetos

O exemplo a seguir demonstra a sintaxe para compartilhar um projeto com uma equipe:
<pre>PUT /attask/api/v9.0/project/123abcxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx/share?accessID=123abcxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx&amp;access&amp;accessObjObjObjObjObjObjObjObjObjObjObjObjObjObjObjObjObjObjObjCode=TEAMOX=TEAXX B</pre>Ao editar um objeto, você pode substituir todas as regras de acesso em um objeto fazendo um PUT e enviando atualizações semelhantes ao seguinte exemplo:
<pre>PUT /attask/api/v9.0/project/123abcxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx?method=PUT&amp;updates={accessRules:[{accessID:'123abcxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx',accessObjCode:'TEAMOB',coreAction:'VIEW'}]}</pre>O exemplo a seguir mostra a sintaxe para mover uma tarefa de um projeto para outro:
...
<pre>DELETE /attask/api/v9.0/task/4c78821c000d6fa8d5e52f07a1d54d0 <br>DELETE /attask/api/v9.0/task?id=4c78821c000d6fa8d5e52f07a1d54d0 <br>DELETE /attask/api/v9.0/task/4c78821c000d6fa8d5e52f07a1d54d0?force=true <br>DELETE /attask/api/v9.0/task?id=4c78821c000d6fa8d5e52f07a1d54d0?force=true</pre>
...
<pre>PUT /attask/api/v9.0/proj?updates=[{"name":"Test_Project_1"},{"name":"Test_Project_2"}]&amp;method=POST&amp;apiKey=123ab-cxxxxxxxxxxxxxxxxxxxxxxxxxxxxx</pre>que resulte em um retorno semelhante ao seguinte:
...
<pre>PUT /attask/api/v9.0/proj?Umethod=PUT&amp;updates=[{"ID":"123abcxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx","name":"Test_Project_1_ Edit"},{"ID":"123abcxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx","name":"Test_Project_2_Edit"}]&amp;apiKey=123abcxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx</pre>que resulte em um retorno semelhante ao seguinte:
